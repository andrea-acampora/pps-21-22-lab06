...
trait NotTwoConsecutive[T] extends Parser[T]:
  ...
  private[this] var previousElem: Option[T] = None
  var isPresent: Boolean = false

  abstract override def parse(t: T): Boolean =  if t != previousElem.orElse(None) then { previousElem = Some(t); super.parse(t)} else false
